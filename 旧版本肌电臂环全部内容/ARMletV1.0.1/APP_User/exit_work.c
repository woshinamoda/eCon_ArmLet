...
void htim9_work_serve()
{
	...
	{//如果接入充电器状态
		...
		{//确定完成初始化以后方可在充电时关闭蓝牙电源和LED
			PCout(14) = 0;									//因为只要拔掉充电器就会重启，默认关掉所有		PWR(IO) = 0
			HAL_UART_DeInit(&huart1);				//关闭串口
			IMU_Flag = 0;										//关闭IMU发送
			HAL_TIM_Base_Stop_IT(&htim10);	//关闭EMG采集
		}
		check_Charge_OK();				//检测是否连续1000次到达4.1V充电电压，确认到达视为充满	
		check_Charge_BySTAT();		//检测是否STAT引脚高电平，有一次高电平视为充满状态
	}//end of PB4 judege
	else
	{//如果没有接入充电器
