...
uint8_t		 Emg_Data_Sent[20]={0xBB,0xAA};								//Emg_Data_Sent
uint8_t		 Emg_With_IMU_sent[30] = {0xBB,0xAA};					//EMG和IMU都有数据
...
void get_ADCdata(void)
{
	...
	/* 郭伟超版本 crcCode校验 */	
	Emg_Data_Sent[16] = number;													//序号位自动+1
}
...
void Get_ADCdata_With_IMU(void)
{
	...
	Emg_With_IMU_sent[0]=0xBB;								
	Emg_With_IMU_sent[1]=0xAA;													//头码
	Emg_With_IMU_sent[2]=18;														//长度
	...
	for(int i=3;i<15;i++)
	{
		Emg_With_IMU_sent[i]=AD_Sent[i-3];								//对应EMG数据传递到 从字节[3] --  [14]
	}	
	
	for(uint8_t i=15; i<21; i++)
	{
		Emg_With_IMU_sent[i] = IMU_Data_buf[i-15];				//对应IMU数据传递到	从字节[15] -- [20]
	}
	
	Emg_With_IMU_sent[21] = 0;
	
	for(uint8_t i=3; i<21;	i++)
	{
		Emg_With_IMU_sent[21] += Emg_With_IMU_sent[i];
	}
	Emg_With_IMU_sent[22] = number;											//序号位自动+1
